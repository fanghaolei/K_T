# This is the fored script from the initial script. 
# This model removed age and gender from predictors and only used "title" as an interaction of both predictors
# The results ouput a test accuracy of .79904 on Kaggle, which was better than the initial model
...
model2<-glm(sur~fam_size+class+title+fare.imputed, family = "binomial", data = train)
model3<-glm(sur~fam_size+class+sex*age.imputed+fare.imputed, family = "binomial", data = train) 
summary(model2)

# apply model 
prediction<-predict.glm(model2, newdata = test, type  = "response")
